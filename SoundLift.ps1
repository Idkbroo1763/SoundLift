...
function Get-SoundLiftThemePalette([string]$themeName = $script:themeName) {
    $theme = switch ($themeName) {
        ...
        'Midnight Blue'     { @{ Accent='#60A5FA'; AccentDark='#2563EB'; Page='#071A33'; Hover='#102E54'; Base='#030A16'; Surface='#091426'; SurfaceAlt='#06101F'; Control='#10213A'; Border='#244466' } }
...
function Get-SoundLiftSavedThemeName {
    try {
        ...
        if (Test-Path -LiteralPath $earlySettingsPath) {
            ...
            $normalized = switch ($name) { 'Black & Red' {'Fekete és piros'} 'Black & Blue' {'Fekete és kék'} 'Graphite & Green' {'Grafit és zöld'} 'Világos' {'Fekete és piros'} 'R6 Siege' {'Carbon Gold'} 'Arctic' {'Fekete és kék'} 'Egyéni téma' {'Fekete és piros'} default { if($name){$name}else{'Fekete és piros'} } }
            return $normalized
        }
    } catch { }
...
function Set-SoundLiftElementTheme([Windows.DependencyObject]$element, [hashtable]$palette) {
    ...
    if ($element -is [Windows.Controls.Primitives.ScrollBar]) {
...
function Set-SoundLiftWindowStyle([Windows.Window]$targetWindow) {
    ...
    $useDarkTitle = $(if($palette.IsLight){0}else{1})
    $targetWindow.Add_SourceInitialized({try{$handle=[Windows.Interop.WindowInteropHelper]::new($targetWindow).Handle;$dark=$useDarkTitle;[void][AudioAppNative]::DwmSetWindowAttribute($handle,20,[ref]$dark,4)}catch{}}.GetNewClosure())
...
function Complete-SoundLiftStartup {
    try {
        ...
        # Sikeres induláskor se maradjanak a process_started és
        # initialization_succeeded események a helyi várólistában.
        # Így a Discord indulási napló azonnal megérkezik, nem csak egy
        # későbbi hiba vagy következő futtatás alkalmával.
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
  ...
  <Window.Resources>
    ...
    <Style x:Key="ThemedContextMenu" TargetType="ContextMenu">
      <Setter Property="Background" Value="{DynamicResource SurfaceBrush}"/><Setter Property="Foreground" Value="{DynamicResource PrimaryTextBrush}"/>
      <Setter Property="BorderBrush" Value="{DynamicResource BorderBrush}"/><Setter Property="BorderThickness" Value="1"/>
      <Setter Property="Padding" Value="5"/>
      <Setter Property="Template">
        <Setter.Value><ControlTemplate TargetType="ContextMenu"><Border Background="{TemplateBinding Background}" BorderBrush="{TemplateBinding BorderBrush}" BorderThickness="{TemplateBinding BorderThickness}" CornerRadius="9" Padding="{TemplateBinding Padding}"><StackPanel IsItemsHost="True" KeyboardNavigation.DirectionalNavigation="Cycle"/></Border></ControlTemplate></Setter.Value>
      </Setter>
    </Style>
    <Style x:Key="ThemedMenuItem" TargetType="MenuItem">
      <Setter Property="Foreground" Value="{DynamicResource PrimaryTextBrush}"/><Setter Property="Background" Value="Transparent"/>
      <Setter Property="Padding" Value="13,9"/><Setter Property="Margin" Value="0,1"/><Setter Property="Cursor" Value="Hand"/>
      <Setter Property="Template">
        <Setter.Value>
          <ControlTemplate TargetType="MenuItem">
            <Border x:Name="MenuItemBorder" Background="{TemplateBinding Background}" CornerRadius="6" Padding="{TemplateBinding Padding}">
              <ContentPresenter ContentSource="Header" RecognizesAccessKey="True" VerticalAlignment="Center"/>
            </Border>
            <ControlTemplate.Triggers><Trigger Property="IsHighlighted" Value="True"><Setter TargetName="MenuItemBorder" Property="Background" Value="{DynamicResource HoverBrush}"/><Setter Property="Foreground" Value="{DynamicResource PrimaryTextBrush}"/></Trigger></ControlTemplate.Triggers>
          </ControlTemplate>
        </Setter.Value>
      </Setter>
    </Style>
  ...
  <Grid Background="{DynamicResource PageGradient}">
    ...
    <Border Grid.Column="0" Margin="14" Background="{DynamicResource SurfaceBrush}" CornerRadius="16" BorderBrush="{DynamicResource BorderBrush}" BorderThickness="1" Effect="{StaticResource CardShadow}">
      <Grid Margin="13">
        ...
        <StackPanel Margin="5,9,5,24">
          <TextBlock Text="✦  SOUNDLIFT" FontFamily="Segoe UI Black" FontSize="18" Foreground="{DynamicResource PrimaryTextBrush}"/>
          <Border Height="3" Width="49" Background="{DynamicResource AccentTextBrush}" CornerRadius="2" HorizontalAlignment="Left" Margin="22,6,0,0"/>
        </StackPanel>

        <StackPanel Grid.Row="1">
          <TextBlock Text="FŐMENÜ" FontSize="9" FontWeight="Bold" Foreground="{DynamicResource MutedTextBrush}" Margin="10,0,0,8"/>
          <Button Content="⌂   Főoldal" Background="{DynamicResource AccentGradient}" BorderBrush="{DynamicResource AccentTextBrush}" BorderThickness="1"/>
          <Button Name="DeviceButton" Content="♫   Hangeszközök" Style="{StaticResource QuickProfileButton}"/>
          <Button Name="HotkeyButton" Content="⌨   Billentyűparancsok" Style="{StaticResource QuickProfileButton}"/>
          <Button Name="DiagnosticsButton" Content="◉   Diagnosztika" Style="{StaticResource QuickProfileButton}"/>
          <Button Name="LicenseButton" Content="◇   Licenc" Style="{StaticResource QuickProfileButton}"/>
          <Button Name="AboutButton" Content="ⓘ   A SoundLiftről" Style="{StaticResource QuickProfileButton}"/>
          <Button Name="PrivacyButton" Content="◈   Adatvédelem" Style="{StaticResource QuickProfileButton}"/>
          <Button Name="ChangelogButton" Content="≡   Újdonságok" Style="{StaticResource QuickProfileButton}"/>
        </StackPanel>
                ...
                <Grid>
                  ...
                  <Popup Name="PART_Popup" Placement="Bottom" IsOpen="{TemplateBinding IsDropDownOpen}" AllowsTransparency="True" Focusable="False" PopupAnimation="Fade">
                    <Border Margin="0,3,0,0" MinWidth="{TemplateBinding ActualWidth}" MaxHeight="310" Background="{DynamicResource SurfaceBrush}" BorderBrush="{DynamicResource BorderBrush}" BorderThickness="1" CornerRadius="7">
                      <ScrollViewer Margin="3" VerticalScrollBarVisibility="Hidden" HorizontalScrollBarVisibility="Hidden" PanningMode="VerticalOnly"><StackPanel IsItemsHost="True" KeyboardNavigation.DirectionalNavigation="Contained"/></ScrollViewer>
                    </Border>
                  </Popup>
                </Grid>
              </ControlTemplate>
    ...
    <Grid Grid.Column="1" Margin="4,14,14,14">
      ...
      <Grid>
        ...
        <StackPanel VerticalAlignment="Center">
          <TextBlock Text="JOBB HANGZÁS." FontFamily="Segoe UI Black" FontSize="16" Foreground="{DynamicResource PrimaryTextBrush}"/>
      ...
      <ScrollViewer Grid.Row="1" VerticalScrollBarVisibility="Hidden" HorizontalScrollBarVisibility="Disabled" PanningMode="VerticalOnly">
        <StackPanel>
          <Grid Margin="0,0,0,12">
            <Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="12"/><ColumnDefinition Width="252"/></Grid.ColumnDefinitions>
              ...
              <Grid>
                ...
                <Grid Width="184" Height="184" VerticalAlignment="Center">
                  ...
                  <StackPanel VerticalAlignment="Center" HorizontalAlignment="Center">
                    <TextBlock Name="VolumeValue" Text="100%" FontFamily="Segoe UI Black" FontSize="37" HorizontalAlignment="Center"/>
                    <TextBlock Text="HANGERŐ" FontSize="9" FontWeight="Bold" Foreground="{DynamicResource MutedTextBrush}" HorizontalAlignment="Center"/>
                  </StackPanel>
                </Grid>
                <StackPanel Grid.Column="2" VerticalAlignment="Center">
                  <DockPanel><TextBlock Text="Hangerő-erősítés" FontSize="14" FontWeight="Bold"/><TextBlock Text="max. 300%" HorizontalAlignment="Right" FontSize="10" Foreground="{DynamicResource MutedTextBrush}"/></DockPanel>
                  <Slider Name="VolumeSlider" Minimum="0" Maximum="300" Value="100" TickFrequency="5" IsSnapToTickEnabled="True"/>
                  <DockPanel Margin="0,10,0,0"><TextBlock Text="Mélyhangkiemelés" FontSize="14" FontWeight="Bold"/><TextBlock Name="BassValue" Text="6 dB" HorizontalAlignment="Right" FontSize="13" FontWeight="Bold" Foreground="{DynamicResource AccentTextBrush}"/></DockPanel>
                  <Slider Name="BassSlider" Minimum="0" Maximum="24" Value="6" TickFrequency="1" IsSnapToTickEnabled="True"/>
                  <DockPanel Margin="0,10,0,0"><TextBlock Text="Basszus karaktere" FontSize="14" FontWeight="Bold"/><TextBlock Name="FrequencyValue" Text="75 Hz" HorizontalAlignment="Right" FontSize="13" FontWeight="Bold" Foreground="{DynamicResource AccentTextBrush}"/></DockPanel>
                  <Slider Name="FrequencySlider" Minimum="40" Maximum="160" Value="75" TickFrequency="5" IsSnapToTickEnabled="True"/>
                  ...
                  <Button Name="ApplyButton" Content="BEÁLLÍTÁSOK ALKALMAZÁSA" Style="{StaticResource PrimaryButton}" Margin="0,8,0,0"/>
                </StackPanel>
              </Grid>
            </Border>

            <Border Grid.Column="2" Style="{StaticResource DashboardCard}" Padding="13">
              <Grid>
                <Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="*"/><RowDefinition Height="Auto"/></Grid.RowDefinitions>
                <DockPanel Margin="3,0,3,8">
                  <TextBlock Text="GYORS PROFILOK" FontSize="10" FontWeight="Bold" Foreground="{DynamicResource SectionTextBrush}"/>
                  <TextBlock Name="ActiveProfileText" Text="Aktív: Egyéni" HorizontalAlignment="Right" FontSize="10" Foreground="{DynamicResource AccentTextBrush}"/>
                </DockPanel>
                <ScrollViewer Grid.Row="1" VerticalScrollBarVisibility="Hidden">
                  <StackPanel Name="ProfilePanel">
                    ...
                    <TextBlock Name="CustomFeaturesTitle" Text="EGYEDI FUNKCIÓK" FontSize="9" FontWeight="Bold" Foreground="{DynamicResource SectionTextBrush}" Margin="4,8,0,5" Visibility="Collapsed"/>
                    ...
                    <Button Name="ProfileOrderButton" Content="+   Profilok rendezése" Style="{StaticResource UtilityButton}" Margin="0,3,0,0"/>
                  </StackPanel>
                </ScrollViewer>
                <TextBlock Grid.Row="2" Text="A profilok mindig elérhetők ezen a panelen." FontSize="9" Foreground="{DynamicResource MutedTextBrush}" TextWrapping="Wrap" Margin="3,8,3,0"/>
              </Grid>
            </Border>
          </Grid>

          <Border Style="{StaticResource DashboardCard}" Margin="0,0,0,12" Padding="18,13">
            <StackPanel>
              <DockPanel Margin="0,0,0,8">
                <TextBlock Text="10 SÁVOS HANGSZÍNSZABÁLYZÓ" FontSize="10" FontWeight="Bold" Foreground="{DynamicResource SectionTextBrush}"/>
                <StackPanel Orientation="Horizontal" HorizontalAlignment="Right">
                  <TextBlock Name="LiveBoostText" Text="Erősítés: 0,0 dB" FontSize="10" Foreground="{DynamicResource MutedTextBrush}" Margin="0,0,12,0"/>
                  <TextBlock Name="LiveClipText" Text="NINCS JEL" FontSize="10" FontWeight="Bold" Foreground="{DynamicResource MutedTextBrush}"/>
                </StackPanel>
              </DockPanel>
              <Grid>
                <Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="145"/></Grid.ColumnDefinitions>
                <Border Background="{DynamicResource SurfaceAltBrush}" CornerRadius="11" Padding="9">
                  <UniformGrid Name="EqPanel" Rows="1" Columns="10"/>
                </Border>
                <StackPanel Grid.Column="1" Margin="14,3,0,0">
                  <TextBlock Text="ÉLŐ HANGMÉRŐ" FontSize="9" FontWeight="Bold" Foreground="{DynamicResource SectionTextBrush}" Margin="0,0,0,9"/>
                  <TextBlock Text="BAL" FontSize="8" Foreground="{DynamicResource MutedTextBrush}"/>
                  <ProgressBar Name="LeftPeakMeter" Height="8" Minimum="0" Maximum="100" Value="0" Foreground="{DynamicResource AccentTextBrush}" Background="{DynamicResource ControlBrush}"/>
                  <TextBlock Name="LeftPeakText" Text="0%" FontSize="9" HorizontalAlignment="Right" Foreground="{DynamicResource SecondaryTextBrush}"/>
                  <TextBlock Text="JOBB" FontSize="8" Foreground="{DynamicResource MutedTextBrush}" Margin="0,8,0,0"/>
                  <ProgressBar Name="RightPeakMeter" Height="8" Minimum="0" Maximum="100" Value="0" Foreground="{DynamicResource AccentTextBrush}" Background="{DynamicResource ControlBrush}"/>
                  <TextBlock Name="RightPeakText" Text="0%" FontSize="9" HorizontalAlignment="Right" Foreground="{DynamicResource SecondaryTextBrush}"/>
                </StackPanel>
              </Grid>
            </StackPanel>
          </Border>
          ...
          <UniformGrid Columns="4" Margin="0,0,0,12">
            <Border Style="{StaticResource DashboardCard}" Padding="13" Margin="0,0,8,0"><StackPanel><TextBlock Text="▤" FontSize="19" Foreground="{DynamicResource AccentTextBrush}"/><TextBlock Text="Profilkezelés" FontWeight="Bold" Margin="0,5,0,2"/><Button Name="ProfileManagerButton" Content="Megnyitás" Style="{StaticResource UtilityButton}" Margin="0,8,0,0"/><Button Name="SaveButton" Visibility="Collapsed"/><Button Name="LoadButton" Visibility="Collapsed"/><Button Name="ExportButton" Visibility="Collapsed"/><Button Name="ImportButton" Visibility="Collapsed"/></StackPanel></Border>
            <Border Style="{StaticResource DashboardCard}" Padding="13" Margin="0,0,8,0"><StackPanel><TextBlock Text="▣" FontSize="19" Foreground="{DynamicResource AccentTextBrush}"/><TextBlock Text="Hangteszt" FontWeight="Bold" Margin="0,5,0,2"/><Button Name="TestButton" Content="60 Hz teszt" Style="{StaticResource UtilityButton}" Margin="0,8,0,0"/></StackPanel></Border>
            <Border Style="{StaticResource DashboardCard}" Padding="13" Margin="0,0,8,0"><StackPanel><TextBlock Text="✚" FontSize="19" Foreground="{DynamicResource AccentTextBrush}"/><TextBlock Text="Automatikus javítás" FontWeight="Bold" Margin="0,5,0,2"/><Button Name="RepairApoButton" Content="APO javítása" Style="{StaticResource UtilityButton}" Margin="0,8,0,0"/></StackPanel></Border>
            <Border Style="{StaticResource DashboardCard}" Padding="13"><StackPanel><TextBlock Text="↻" FontSize="19" Foreground="{DynamicResource AccentTextBrush}"/><TextBlock Text="Frissítés" FontWeight="Bold" Margin="0,5,0,2"/><Button Name="UpdateButton" Content="Keresés" Style="{StaticResource UtilityButton}" Margin="0,8,0,0"/></StackPanel></Border>
          </UniformGrid>
...
function Show-DiagnosticsWindow {
    ...
    $reportBox.VerticalScrollBarVisibility = 'Hidden'; $reportBox.HorizontalScrollBarVisibility = 'Hidden'
...
function Show-ProblemReportWindow {
    ...
    $description=[Windows.Controls.TextBox]::new(); $description.Height=78; $description.MaxLength=1000; $description.AcceptsReturn=$true; $description.TextWrapping='Wrap'; $description.VerticalScrollBarVisibility='Hidden'; $description.Padding=12; $description.Background=$window.Resources['SurfaceBrush']; $description.Foreground=$window.Resources['PrimaryTextBrush']; $description.BorderBrush=$window.Resources['BorderBrush']; $description.BorderThickness=1; $description.Margin=[Windows.Thickness]::new(0,0,0,15)
    ...
    $box = [Windows.Controls.TextBox]::new(); $box.IsReadOnly=$true; $box.AcceptsReturn=$true; $box.TextWrapping='Wrap'; $box.VerticalScrollBarVisibility='Hidden'; $box.HorizontalScrollBarVisibility='Hidden'; $box.FontFamily='Consolas'; $box.FontSize=12; $box.Padding=14; $box.Background=$window.Resources['SurfaceBrush']; $box.Foreground=$window.Resources['PrimaryTextBrush']; $box.BorderBrush=$window.Resources['BorderBrush']; $box.BorderThickness=1
    $refreshPreview = {
...
function Show-DeveloperConsole {
    ...
    $console=[Windows.Controls.TextBox]::new();$console.IsReadOnly=$true;$console.AcceptsReturn=$true;$console.TextWrapping='Wrap';$console.VerticalScrollBarVisibility='Hidden';$console.HorizontalScrollBarVisibility='Hidden';$console.Background='#050505';$console.Foreground='#D1FAE5';$console.FontFamily='Consolas';$console.FontSize=12;$console.Padding=[Windows.Thickness]::new(12)
    $refresh={
...
function Show-LicenseManagerWindow {
    ...
    $scroll=[Windows.Controls.ScrollViewer]::new();$scroll.VerticalScrollBarVisibility='Hidden';$scroll.HorizontalScrollBarVisibility='Hidden';$scroll.PanningMode='VerticalOnly';$panel=[Windows.Controls.StackPanel]::new()
...
function Show-PrivacyWindow {
    ...
    $scroll=[Windows.Controls.ScrollViewer]::new(); $scroll.VerticalScrollBarVisibility='Hidden'; $scroll.HorizontalScrollBarVisibility='Hidden'; $scroll.PanningMode='VerticalOnly'
...
• A Profilkezelés lenyíló menüje most teljesen követi a kiválasztott témát.
...
• A gyors profilok állandóan láthatók a főképernyő jobb oldalán.
• Az Egyéni téma és a hozzá tartozó szerkesztő kikerült; a korábbi Egyéni téma automatikusan Fekete és piros témára áll vissza.
...
• Az egyéni színek automatikusan mentődnek, a szövegkontraszt és a címsor megjelenése pedig világos háttérnél is olvasható marad.

V1.6.4 – LETISZTULT MIKROFONABLAK
• A hosszú Shadow-figyelmeztetés kikerült a mikrofonablakból, így a felület ismét rövid és átlátható.
...
"@
    ...
    $box.TextWrapping = 'Wrap'; $box.VerticalScrollBarVisibility = 'Hidden'; $box.HorizontalScrollBarVisibility = 'Hidden'; $box.FontSize = 14; $box.Padding = [Windows.Thickness]::new(16)
...
function Show-FirstRunWizard {
    ...
    $profileChoice = [Windows.Controls.ComboBox]::new(); $profileChoice.Width=300; $profileChoice.Height=42; $profileChoice.HorizontalAlignment='Left'; $profileChoice.Margin=[Windows.Thickness]::new(0,18,0,0); $profileChoice.Padding=[Windows.Thickness]::new(10,4,8,4); $profileChoice.FontWeight='SemiBold'; $profileChoice.Visibility='Collapsed'
    # A rendszer alap ComboBox-sablonja világos hátteret használ, ezért a
    # világos szöveg több témában eltűnt. Ugyanazt a modern, témakövető
    # sablont és elemszínezést használjuk, mint a főablak témaválasztójánál.
    $profileChoice.Template = $ThemeCombo.Template
    foreach ($resourceKey in @($ThemeCombo.Resources.Keys)) { $profileChoice.Resources[$resourceKey] = $ThemeCombo.Resources[$resourceKey] }
    $profileChoice.Background=$wizard.Resources['ControlBrush']; $profileChoice.Foreground=$wizard.Resources['PrimaryTextBrush']; $profileChoice.BorderBrush=$wizard.Resources['BorderBrush']
...
$ProfileManagerButton.Add_Click({
    ...
    foreach ($resourceName in @('SurfaceBrush','PrimaryTextBrush','BorderBrush','HoverBrush','AccentTextBrush')) {
        $menu.Resources[$resourceName] = $window.Resources[$resourceName]
    }
    $menu.Style = $window.Resources['ThemedContextMenu']; $menu.MinWidth = 230
    foreach ($entry in @(
        ...
        $item.Style=$window.Resources['ThemedMenuItem']
...
function Show-MicrophoneEnhancementWindow {
    ...
    $note=[Windows.Controls.TextBlock]::new();$note.Text='A SoundLift a Windows alapértelmezett mikrofonjának bemeneti hangerejét állítja.';$note.TextWrapping='Wrap';$note.Foreground='#94A3B8';$note.Margin=[Windows.Thickness]::new(0,0,0,16);[void]$root.Children.Add($note)
...
function Show-HotkeyEditor {
    ...
    $save.Tag=[PSCustomObject]@{ Dialog=$dialog; Selectors=@($selectors) }
    $cancel.Add_Click({$dialog.Close()}.GetNewClosure())
    $save.Add_Click({
        param($sender,$eventArgs)
        $save=$sender; $dialog=$sender.Tag.Dialog; $selectors=@($sender.Tag.Selectors)
